...
/**
 * Controller for calculating estimated repair costs based on car model and required operations.
 * 
 * @author jtibke
 */
@RestController
...
public class RepairEstimationController {
	...
	@Operation(
		summary = "Estimate repair costs based on car model and operations",
		description = "This endpoint receives the car model and a list of repair operations, looks up price estimates from a pricing table, and returns the total estimated cost."
	)
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Cost estimation calculated successfully",
			content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = RepairEstimationResponseDto.class))),
		...
		@ApiResponse(responseCode = "500", description = "Internal server error during cost estimation",
			content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDto.class)))
	})
